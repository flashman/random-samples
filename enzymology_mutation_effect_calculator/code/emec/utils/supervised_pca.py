...
class BaseSupervisedPCA(object):
    ...
    References
    ----------
    Bair, Eric, et al. "Prediction by supervised principal components." Journal of the American
    Statistical Association 101.473 (2006).
    """
    ...
    def fit(self, X, y):
        ...
        if len(self._leavouts) == dummy_X.shape[2]:
            raise ValueError(
                "The total number of features to be left out is equal to the "
                "total number of features. Please try with a smaller threshold value."
            )
...
class SupervisedPCARegressor(BaseSupervisedPCA, RegressorMixin):
    ...
    def __init__(
        ...
        super(SupervisedPCARegressor, self).__init__(
...
class SupervisedPCAClassifier(BaseSupervisedPCA, ClassifierMixin):
    ...
    def __init__(
        ...
        super(SupervisedPCAClassifier, self).__init__(
            fit_intercept=fit_intercept,
            model=model,
            threshold=threshold,
            n_components=n_components,
